...
const float bias = 0.29;
float radius;
const vec2 randomSize = vec2(495, 495);
vec2 screenSize = vec2(750, 500);
vec2 kernel[8];
...
float calcAO(vec3 normal){
	...
	float ao = 0.;
	float depth = getDepth(uv);
	vec3 position = getPosition(uv * 2. - 1.);
	radius = 0.5/sqrt(abs(position.z));
	for(int i = 0; i < 8; i++){
...
void main(){
	...
	float ambient = .2 + .43*(4.95 * calcAO(normal) + .25*(1.-shadow));
	...
	//gl_FragColor = vec4(vec3(5.*ambient), 1.);
}
